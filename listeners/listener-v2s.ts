...
import {
  // liquidityStateV4Layout as LIQUIDITY_STATE_LAYOUT_V4,
  ALL_PROGRAM_ID,
  MARKET_STATE_LAYOUT_V3,
  DEVNET_PROGRAM_ID,
  CpmmPoolInfoLayout,
...
export class Listeners extends EventEmitter {
  ...
  private MARKET_PROGRAM_ID = ALL_PROGRAM_ID;
  private OPENBOOK_MARKET: PublicKey = ALL_PROGRAM_ID.OPEN_BOOK_PROGRAM;
  ...
  public async start(config: ListenerConfig | null = null) {
    ...
    if (config.network === 'devnet') {
      this.MARKET_PROGRAM_ID = DEVNET_PROGRAM_ID;
      this.OPENBOOK_MARKET = DEVNET_PROGRAM_ID.OPEN_BOOK_PROGRAM;
    }
      ...
      [
        ...
        },
        {
          memcmp: {
            ...
            bytes: this.OPENBOOK_MARKET.toBase58(),
          },
        },
        {
          memcmp: {
  ...
  private async subscribeToCpmmPools(config: { quoteToken: Token }) {
    return this.connection.onProgramAccountChange(
      this.MARKET_PROGRAM_ID.CREATE_CPMM_POOL_PROGRAM,
      async (updatedAccountInfo) => {
